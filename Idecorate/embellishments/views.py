...
def save_styleboard_template(request):
    ...
    if template_id:
        ...
        template = getTemplateItems(template_id)

        if template and not template.is_used:
            
            form = SaveTemplateForm(initial={'name':template.name,'description':template.description})
